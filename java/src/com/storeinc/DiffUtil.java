...
public class DiffUtil {
	...
	private int checkMatchIndex(String chunkMd5,HashMap<String, ArrayList<Integer>> checksumArray,int lastmatchNo){
	...
	private ArrayList<DiffItem> searchChunk(String strInput,HashMap<String, ArrayList<Integer>> checksumArray,int chunkSize){
	    ...
	    int tLen=strInput.length();
	    	...
	    	while(currentIndex<=tLen){
				...
				int matchTrunkIndex=checkMatchIndex(chunkMd5,checksumArray,lastmatchNo);
	...
	public JSONObject makeIncDataFile(String oldFile,String newFile,int chunkSize){
	  ...
	  //  var oldChecksum=oldFileChecksum("F:/nginx-1.5.1/html/client-1000.js");
	   ...
	   // var diffArray=searchChunk("F:/nginx-1.5.1/html/server.js",oldChecksum);
	    HashMap<String, ArrayList<Integer>> oldChecksum=getOldMd5Map(oldFile,chunkSize);
	    ArrayList<DiffItem> diffArray=searchChunk(newFile,oldChecksum,chunkSize);
	    String arrayData ="";
	   // var newData="";
	    ...
	    for(int i=0;i<size;i++){
			...
			// }
	        	if (item.isMatch) {
					...
					else if(lastitem.isMatch&&Integer.parseInt(lastitem.data)+1==Integer.parseInt(item.data)){
						matchCount++;
					}
					else if(lastitem.isMatch&&Integer.parseInt((lastitem.data+1))!=Integer.parseInt(item.data)){
    ...
    public JSONObject makeIncDataFromFile(String oldFile,String newFile,int chunkSize){
	...
	public static void main(String[] args) {
		...
		JSONObject json=dUtil.makeIncDataFromFile("/Users/waynelu/storeinc/demo/static/hello/dist/1.0.6/main-1.0.6.js", "/Users/waynelu/storeinc/demo/static/hello/dist/1.0.7/main-1.0.7.js", 12);
